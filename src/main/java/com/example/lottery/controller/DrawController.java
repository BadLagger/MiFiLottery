...
public class DrawController {
  ...
  public List<Draw> getActiveDraws() {
    return drawService.findByStatus(DrawStatus.ACTIVE);
  }
  ...
  public ResponseEntity<Draw> createDraw(@Valid @RequestBody DrawRequestDto request)
    ...
    if (request.duration() == null) {
      request =
          new DrawRequestDto(
              request.name(), request.lotteryTypeId(), request.startTime(), defaultDuration);
    } else {
  ...
  public ResponseEntity<Draw> cancelDraw(@PathVariable Long id) {
    ...
    if (existingDraw.getStatus() == DrawStatus.CANCELLED
        || existingDraw.getStatus() == DrawStatus.COMPLETED) {
